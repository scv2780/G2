...
<template>
  <section class="p-6 max-w-4xl mx-auto">
    ...
    <div v-else>
      <div v-for="section in survey.sections" :key="section.section_code" class="mb-6 border p-4 rounded">
        <h3 class="text-xl font-semibold mb-2">{{ section.section_title }}</h3>

        <div v-for="sub in section.subsections" :key="sub.subsection_code" class="mb-4 pl-4 border-l">
          <h4 class="font-medium mb-2">{{ sub.subsection_title }}</h4>

          <div v-for="item in sub.items" :key="item.item_code" class="mb-3">
            <label class="block font-medium mb-1">{{ item.question_text }}</label>
            ...
            <div v-else-if="item.question_type === 'RADIO'" class="space-y-1">
              <label v-for="opt in item.option_values" :key="opt.value" class="flex items-center gap-2">
                <input type="radio" :name="'item_' + item.item_code" :value="opt.value" v-model="answers[item.item_code]" />
                <span>{{ opt.label }}</span>
              </label>
            </div>

            <!-- CHECKBOX (v-model = array) -->
            <div v-else-if="item.question_type === 'CHECKBOX'" class="space-y-1">
              <label v-for="opt in item.option_values" :key="opt.value" class="flex items-center gap-2">
                <input type="checkbox" :value="opt.value" v-model="answers[item.item_code]" />
                <span>{{ opt.label }}</span>
              </label>
            </div>
          </div>
      ...
      <div class="text-right mt-6">
        <button @click="submitSurvey" class="border px-4 py-2 rounded bg-black text-white">
          제출하기
        </button>
      </div>
    </div>
